...
{
    public class Ldelem_Any : Instruction
	{/// <summary>
	 ...
	 /// <param name="arrayRef">Array reference.</param>
		public Ldelem_Any(Class type, Register result, Register index, Register arrayRef) : base("Ldelem_Any", result, new Operand[] { arrayRef, index })
		{
			this.type = type;
		}
		...
		public override void Process(Method method)
		{
			if (this.type.IsClass || this.type.IsValueType)
			{
				...
				throw new NotImplementedEngineException("Type " + this.Type + " can't be handled by Ldelem_Any");
		}
	}
}
